...
// --- Mood Options ---
const moodOptions = [ /* ... moodOptions array ... */
    { emoji: '😄', label: 'Happy', lottieUrl: 'https://fonts.gstatic.com/s/e/notoemoji/latest/1f604/lottie.json' }, { emoji: '🤩', label: 'Excited', lottieUrl: 'https://fonts.gstatic.com/s/e/notoemoji/latest/1f929/lottie.json' }, { emoji: '😊', label: 'Calm', lottieUrl: 'https://fonts.gstatic.com/s/e/notoemoji/latest/1f60a/lottie.json' }, { emoji: '😟', label: 'Sad', lottieUrl: 'https://fonts.gstatic.com/s/e/notoemoji/latest/1f61f/lottie.json' }, { emoji: '😔', label: 'Worried', lottieUrl: 'https://fonts.gstatic.com/s/e/notoemoji/latest/1f614/lottie.json' }
];

// --- Mood Scoring Map ---
const moodScores = { /* ... moodScores map ... */
    'Happy': 5, 'Excited': 4, 'Calm': 4, 'Worried': 2, 'Sad': 1,
};

// --- Mini Courses Data ---
const miniCourses = [ /* ... miniCourses array ... */
    { id: 'med101', title: 'Intro to Meditation', description: 'Learn the basics of mindfulness in 5 minutes.', icon: 'brain', color: colors.courseBlue }, { id: 'stress1', title: 'Stress Relief Breaths', description: 'Quick techniques to calm your nerves.', icon: 'leaf-circle-outline', color: colors.featureGreen }, { id: 'gratitude1', title: 'Gratitude Practice', description: 'Cultivate positivity with a simple daily exercise.', icon: 'heart-outline', color: colors.courseOrange }, { id: 'sleep1', title: 'Better Sleep Tips', description: 'Simple habits for more restful nights.', icon: 'power-sleep', color: colors.coursePurple },
];
...
const HomeScreen = () => {
    ...
    // --- Functions ---
    // Reverted fetchUserName to Namespaced Syntax
    const fetchUserName = useCallback(async () => {
        ...
        if (currentUser) {
            try {
                ...
                // *** Using userDocument.exists PROPERTY ***
    ...
    const analyzeMoodsAndScore = (moodHistory) => { /* ... analyzeMoodsAndScore function ... */
    ...
    // Reverted fetchWellnessData to Namespaced Syntax
    ...
    // Reverted Streak Check Logic to Namespaced Syntax
    const checkAndUpdateStreak = useCallback(async () => {
        ...
        try {
            ...
            // *** Using statsDoc.exists PROPERTY ***
            ...
            // Check if it's a valid Timestamp before calling toDate()
            ...
            if (newStreak !== currentStreak || !lastActivityDate || !isSameDay(lastActivityDate, today)) {
                 ...
                 // Use namespaced set/merge and FieldValue
                 await statsRef.set({
    ...
    useFocusEffect(useCallback(() => { console.log("HomeScreen focused"); fetchUserName(); fetchWellnessData(); checkAndUpdateStreak(); return () => { console.log("HomeScreen blurred"); }; }, [fetchUserName, fetchWellnessData, checkAndUpdateStreak]));
    // -------------------------------------

    const handleMoodSelect = (mood) => { /* ... handleMoodSelect function ... */
        setPendingMood(mood); setShowMoodOptions(false); setConfirmationVisible(false);
        setNote(''); setShowAddNoteView(true);
    };

    // --- Save Mood and Note Handler (Reverted to Namespaced Syntax) ---
    const handleSaveMoodAndNote = async () => {
        ...
        // Check for first mood before the transaction
        ...
        try {
            ...
            await firestore().runTransaction(async (transaction) => { // Namespaced runTransaction
                // 1. Save Mood
                const newMoodRef = moodHistoryCollectionRef.doc(); // Get new doc ref
                transaction.set(newMoodRef, moodData); // Use transaction set

                // 2. Update Points
                 // Use namespaced FieldValue.increment() and FieldValue.serverTimestamp()
                ...
                if (isFirstMoodEver) {
                    const achievementDoc = await transaction.get(firstMoodAchievementRef); // Namespaced transaction get
                     // *** Use achievementDoc.exists PROPERTY ***
                    if (!achievementDoc.exists || !achievementDoc.data()?.earned) {
                        console.log(`Unlocking achievement: ${FIRST_MOOD_ACHIEVEMENT_ID}`);
                        // Use namespaced FieldValue.serverTimestamp()
                        transaction.set(firstMoodAchievementRef, { id: FIRST_MOOD_ACHIEVEMENT_ID, name: FIRST_MOOD_ACHIEVEMENT_NAME, earned: true, earnedAt: firestore.FieldValue.serverTimestamp() }, { merge: true });
                    }
                }
                // 4. Update Stats
                 // Use namespaced FieldValue.increment() and FieldValue.serverTimestamp()
        ...
        switch (moodToSave) { /* ... set suggestions ... */
        ...
        Animated.spring(moodAnimation, { toValue: 1, friction: 4, useNativeDriver: true }).start(() => { Animated.spring(moodAnimation, { toValue: 0, friction: 4, useNativeDriver: true }).start(); });
        setConfirmationVisible(true); setPendingMood(null);
        fetchWellnessData(); // Refresh wellness score display
    };
    // -----------------------------------------

    // --- Other Handlers ---
    const handleDynamicButtonClick = () => { /* ... handleDynamicButtonClick ... */
        if (dynamicButtonText === "Chat with AI") { navigation.navigate('ChatScreen'); }
        else if (dynamicButtonText === "Mindful Breath") { navigation.navigate('MindfulBreath'); }
    };
    const handleReload = () => { /* ... handleReload ... */
    ...
    const handleMoodTrackerNavigation = () => { /* ... handleMoodTrackerNavigation ... */
        navigation.navigate('MoodTrackerScreen');
    };
    const handleProfileNavigation = () => { /* ... handleProfileNavigation ... */
        navigation.navigate('ProfileScreen');
    };
    const handleNotificationNavigation = () => { /* ... handleNotifivationNavigation ... */
        navigation.navigate('NotificationScreen');
    };
    const handleCoursePress = (course) => { /* ... handleCoursePress ... */
        console.log("Course pressed:", course.title);
        Alert.alert("Coming Soon!", `"${course.title}" mini-course is under development.`);
    };
    // ---------------------------------

    // --- Helper to get Wellness Score Color ---
    const getWellnessScoreColor = (score) => { /* ... getWellnessScoreColor function ... */
            ...
            <ScrollView
                ...
                showsVerticalScrollIndicator={false}
                 refreshControl={ // Keep RefreshControl
                    <RefreshControl refreshing={isLoadingName || isLoadingWellness} onRefresh={fetchWellnessData} tintColor={colors.primary} />
                 }
            >
                {/* Header */}
                ...
                <View style={styles.moodCard}>
                     {/* ... Mood Card Content ... */}
                      {/* Mood Header */}
                    {!confirmationVisible && ( <View style={styles.moodHeader}>
                            ...
                            <TouchableOpacity onPress={handleMoodTrackerNavigation} style={styles.arrowButton}><Icon name="arrow-right" size={24} color={colors.textDark} /></TouchableOpacity>
                        </View> )}
                    {/* Mood Options */}
                    {showMoodOptions && !showAddNoteView && !confirmationVisible && ( <>
                            <View style={styles.moodOptionsRow}>{moodOptions.slice(0, 3).map((mood) => ( <TouchableOpacity key={mood.label} style={styles.moodButton} onPress={() => handleMoodSelect(mood.label)}><View style={styles.moodButtonContent}><LottieView style={styles.moodLottie} source={{ uri: mood.lottieUrl }} autoPlay loop /><Text style={styles.moodLabel}>{mood.label}</Text></View></TouchableOpacity> ))} </View>
                            <View style={[styles.moodOptionsRow, { justifyContent: 'center' }]}>{moodOptions.slice(3).map((mood) => ( <TouchableOpacity key={mood.label} style={styles.moodButton} onPress={() => handleMoodSelect(mood.label)}><View style={styles.moodButtonContent}><LottieView style={styles.moodLottie} source={{ uri: mood.lottieUrl }} autoPlay loop /><Text style={styles.moodLabel}>{mood.label}</Text></View></TouchableOpacity> ))} </View>
                        </> )}
                    {/* Add Note View */}
                    {showAddNoteView && !confirmationVisible && ( <View style={styles.addNoteContainer}>
                            <View style={styles.addNoteHeaderContent}><Text style={styles.addNoteHeaderText}>Feeling: {pendingMood}</Text><LottieView style={styles.addNoteHeaderLottie} source={{ uri: moodOptions.find(m => m.label === pendingMood)?.lottieUrl }} autoPlay loop={false} /></View>
                            <TextInput style={styles.noteInput} placeholder="Add a note about your mood (optional)..." placeholderTextColor={colors.textSecondary} value={note} onChangeText={setNote} multiline={true} numberOfLines={3} scrollEnabled={true} />
                            <View style={styles.addNoteActionsContainer}><TouchableOpacity style={styles.skipNoteButton} onPress={handleSaveMoodAndNote}><Text style={styles.skipNoteButtonText}>Skip Note</Text></TouchableOpacity><TouchableOpacity style={styles.saveNoteButton} onPress={handleSaveMoodAndNote}><Text style={styles.saveNoteButtonText}>Save Mood</Text></TouchableOpacity></View>
                        </View> )}
                    {/* Confirmation Card */}
                    {confirmationVisible && ( <View style={styles.confirmationCard}>
                            <TouchableOpacity onPress={handleReload} style={styles.reloadButton}><Icon name="reload" size={24} color={colors.iconGrey} /></TouchableOpacity>
                            <Icon name="check-circle" size={30} color={colors.featureGreen} style={styles.checkmarkIcon} />
                            <Text style={styles.confirmationText}>You're feeling {selectedMood}!</Text><Text style={styles.checkInCompleteText}>Check-in complete!</Text>
                            {suggestionText ? <Text style={styles.suggestionText}>{suggestionText}</Text> : null}
                            {dynamicButtonText && ( <TouchableOpacity onPress={handleDynamicButtonClick} style={styles.dynamicButton}><Text style={styles.dynamicButtonText}>{dynamicButtonText}</Text></TouchableOpacity> )}
                        </View> )}
                </View>
...
const styles = StyleSheet.create({ /* ... styles object ... */
    container: { flex: 1 },
    scrollContainer: { paddingHorizontal: 20, paddingTop: 20, paddingBottom: 120, flexGrow: 1 },
    ...
    userName: { fontSize: 30, color: colors.textDark, fontWeight: '700', marginTop: 4 },
    ...
    moodCard: { backgroundColor: colors.moodYellow, borderRadius: 20, padding: 20, borderWidth: 1, borderColor: '#F5F5DC', minHeight: 250, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.1, shadowRadius: 3, elevation: 2, marginBottom: 30 },
    ...
    moodOptionsRow: { flexDirection: 'row', justifyContent: 'space-around', marginBottom: 10, marginTop: 5, alignItems: 'flex-start' },
    moodButton: { backgroundColor: colors.moodButtonBg, paddingVertical: 10, paddingHorizontal: 10, borderRadius: 18, borderWidth: 1, borderColor: colors.moodButtonBorder, marginHorizontal: 3, minWidth: 85, maxWidth: width * 0.28, alignItems: 'center', shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 2, elevation: 2 },
    ...
    addNoteContainer: { marginTop: 15, alignItems: 'center', paddingHorizontal: 5 },
    ...
    confirmationCard: { backgroundColor: colors.cardBackground, borderRadius: 20, padding: 25, paddingTop: 50, alignItems: 'center', position: 'relative', marginTop: 15, borderWidth: 1, borderColor: colors.lightBorder, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.1, shadowRadius: 3, elevation: 2, minHeight: 250, justifyContent: 'center' },
    ...
    coursesScrollView: { paddingRight: 20 },
    courseCard: { width: width * 0.65, borderRadius: 15, marginRight: 15, padding: 18, flexDirection: 'column', justifyContent: 'space-between', shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.15, shadowRadius: 3, elevation: 4 },
    ...
    loadingIndicator: { marginVertical: 20, height: 50 }, // Added style
});
